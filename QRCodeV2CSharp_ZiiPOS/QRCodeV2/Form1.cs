...
namespace QRCodeV2
{
    public partial class Form1 : Form
    {
        ...
        {
            string POSDomain = "ziicloud.com";
            string QRAppURL = "https://app." + POSDomain + "/store/e/";
            string tableQRURLFormat = QRAppURL + branchIDBox.Text + "?tableNo=";
        ...
        {
            string POSDomain = "ziicloud.com";
            string OnlineQRAppURL = "https://app." + POSDomain + "/store/view/";

            string OnlineQRURLFormat = OnlineQRAppURL ;
        ...
        private void TKGButton_Click(object sender, EventArgs e)
        {
            string POSDomain = "ziicloud.com";
            string takeAeayQRAppURL = "https://app." + POSDomain + "/store/e/";

            //string TakeAwayURLFormat = "https://app.myposzero.com.au/store/e/";
            string TakeAwayURLFormat = takeAeayQRAppURL;
            var folderPath = TKPathBox.Text;
            string path = @folderPath;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            string filePathpng = TKPathBox.Text + "\\Takeaway_" + TakeAwayBranchIDBox.Text + ".png";
            string QRString = TakeAwayURLFormat + TakeAwayBranchIDBox.Text+ "?takeaway=1";
            QRCodeGenerator qrGenerator = new QRCodeGenerator();
            QRCodeData qrCodeData = qrGenerator.CreateQrCode(QRString, QRCodeGenerator.ECCLevel.M);
            PngByteQRCode qrCode = new PngByteQRCode(qrCodeData);
            byte[] qrCodeAsPngByteArr = qrCode.GetGraphic(20);
            ...
            MessageBox.Show("Create Take Away QR code complete");
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                TKPathBox.Text = folderDlg.SelectedPath;
                Environment.SpecialFolder root = folderDlg.RootFolder;
            }
        }

        private void LoyaltyPathSelectButton_Click(object sender, EventArgs e)
        {
            FolderBrowserDialog folderDlg = new FolderBrowserDialog();
            folderDlg.ShowNewFolderButton = true;
            // Show the FolderBrowserDialog.  
            ...
            if (result == DialogResult.OK)
            {
                LoyaltyPathBox.Text = folderDlg.SelectedPath;
                Environment.SpecialFolder root = folderDlg.RootFolder;
            }
        }

        private void LoyaltyGenerateButton_Click(object sender, EventArgs e)
        {
            string POSDomain = "ziicloud.com";
            string LoyaltyQRURL = "https://app." + POSDomain + "/loyalty/guide?merchantId=";
            string ZiiPOSLoyaltyFormat = "https://app.ziicloud.com/loyalty/guide?merchantId=";
            //string LoyaltyURLFormat = "https://app.myposzero.com.au/loyalty/dashboard/";
            string LoyaltyURLFormat = LoyaltyQRURL;
            var folderPath = LoyaltyPathBox.Text;
            string path = @folderPath;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            string filePathpng = LoyaltyPathBox.Text + "\\Loyalty_" + LoyaltyMerchantBox.Text + ".png";
            string QRString = LoyaltyURLFormat + LoyaltyMerchantBox.Text;
            QRCodeGenerator qrGenerator = new QRCodeGenerator();
            QRCodeData qrCodeData = qrGenerator.CreateQrCode(QRString, QRCodeGenerator.ECCLevel.M);
            PngByteQRCode qrCode = new PngByteQRCode(qrCodeData);
            byte[] qrCodeAsPngByteArr = qrCode.GetGraphic(20);
            ...
            MessageBox.Show("Create Loyalty QR code complete");
        }
    }
}
